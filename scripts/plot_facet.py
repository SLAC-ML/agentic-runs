...
def panel_phase(fig, gridspec, row, campaigns, number, ylabel, ylim, ytick):
    ...
    for column, campaign in enumerate(campaigns):
        ...
        ax.plot(x, envelope, color=SERIES, linewidth=1.6, zorder=3)
...
def main(out: Path) -> None:
    ...
    gs = fig.add_gridspec(3, 5, height_ratios=[1.30, 1.0, 0.82], hspace=0.70,
                          wspace=0.26, left=0.105, right=0.985, top=0.885, bottom=0.075)
    ...
    recolour_legend(fig.legend(
        ...
        loc="lower center", bbox_to_anchor=(0.54, -0.012), ncol=4, frameon=False,
        fontsize=6.8, handlelength=1.2, handletextpad=0.5, columnspacing=1.8))
